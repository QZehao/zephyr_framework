 ...
 * @note 重复 stop 是安全的——若模块状态不是 RUNNING，返回 0 且不调业务 stop
 */
...
标准版与商业版（PRO）统一入口，通过 `module_manager_compat.h` 提供抽象层。

```c
#include <modules/module_manager_compat.h>

// 头文件
// - 标准版：直接映射到 module_manager_* 函数
// - PRO 版：映射到 module_manager_pro_* 函数

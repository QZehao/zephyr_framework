...
Zephyr 自动合并的 overlay 须为 **`.overlay`** 后缀，并由 **`configuration_files.cmake`** 按目录与板名解析（详见 **[设备树与内存配置手册.md](../40-应用开发/44-设备树与内存配置手册.md)**）。常见用法：
...
- 本仓库：**[设备树与内存配置手册.md](../40-应用开发/44-设备树与内存配置手册.md)**  

...
**阅读提示**：若尚未理解 **`west build`**、`prj.conf` 与 **`ZEPHYR_BASE`**，请先阅读 **[文档索引.md](../00-入门/02-文档索引.md)** 与 **[独立应用构建说明.md](../10-环境与构建/12-独立应用构建说明.md)**；本手册偏板级与链接细节，可按需查阅章节。
...
CONFIG_EVENT_MAX_TYPES=8      # 关键：从 256 降至 8
...
1. **条件编译**：系统服务和示例模块自动跳过编译
2. **栈极限缩减**：所有栈大小降至最小可行值
3. **事件系统精简**：类型、订阅者、队列全部最小化
4. **禁用调试输出**：完全关闭 printk/console/serial
...
并与 **[项目配置项说明.md](42-项目配置项说明.md)** 中模块相关项一并收紧；改后务必 **`west build ... -p always`** 并查看 **map** 确认余量。
...
- 本仓库：**[开发者入门指南.md](../00-入门/04-开发者入门指南.md)**、**[项目配置项说明.md](42-项目配置项说明.md)**  
